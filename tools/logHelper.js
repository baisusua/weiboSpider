...
if(config.log_infor.appenders){  
    var baseDir = path.resolve(__dirname, '../logs');
    var defaultAtt = config.log_infor["customDefaultAtt"];  
  
    for(var i= 0, j=config.log_infor.appenders.length; i<j; i++){  
        var item = config.log_infor.appenders[i];  
...
// 目录创建完毕，才加载配置，不然会出异常  
log4js.configure(config.log_infor);  
